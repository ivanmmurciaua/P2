...
void fieldOk(vector <int> &v,const Farm &f){
  ...
  for(size_t i=0;i<v.size();i++){
    if(f.fields[v[i]].androids.size()>0){
      ...
      if(androidspeedplus>=f.fields[v[i]].products)                //if the sum stored in androidspeedplus is higher or equal to the products, puts these field into garbage vector
        garbage.push_back(i);
    }
  }
...
void distributeAndroids(Farm &farm){
  ...
  if(dif0(farm)){                                           //check that there are at least one field with products else exit
    while(farm.androids.size()>0 && v.size()>0 && androidsWhoCanWork(farm)){
      ...
      min=minAndr(v,farm);
      fieldOk(v,farm);                                               // comprobamos si los androides están agustico en el campo o no
      for(size_t i=0;i<v.size();i++){
...
bool readFarmData(Farm &farm, int &nextSerialNumber,int &hour){
  ...
  if(fichleer.is_open()){                                                                         //open the file
    ...
    //farm.name=fl.name;                                                                       //read farm
    nextSerialNumber=fl.nextSerialNumber;   hour=fl.hour;                                     //
